<template>
  <tr class="bg-white border-b h-24">
    ...
    <td class="px-6">
      <div>
        <div class="font-semibold flex items-center text-blue-500">
          <svg class="w-4 h-4 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round"
                  :d="shortcut.action.value.svg" />
          </svg>

          {{ shortcut.action.value.name }}
        </div>
        <div v-if="shortcut.action.strategy.instruction.trim() !== ''" class="flex items-start">
          <div class="w-4 mt-0.5 mr-1">
          ...
          <p>{{ shortcut.action.strategy.instruction }}</p>
        </div>
